...
- [ ] Define `User` model (id, name, email, passwordHash, role, createdAt, updatedAt)
- [ ] Define `Organization` model (id, name, slug, settings JSON, createdAt)
- [ ] Define `Driver` model (id, name, email, phone, licenseNumber, status, organizationId, userId?, createdAt, updatedAt)
- [ ] Define `Vehicle` model (id, plateNumber, make, model, year, type, status, organizationId, createdAt, updatedAt)
- [ ] Define `Trip` model (id, vehicleId, driverId, origin, destination, distance, startTime, endTime, status, organizationId, createdAt, updatedAt)
- [ ] Define `MaintenanceRecord` model (id, vehicleId, type, description, scheduledAt, completedAt, cost, status, organizationId, createdAt)
- [ ] Define `FuelLog` model (id, vehicleId, driverId, liters, costPerLiter, totalCost, odometer, station, organizationId, createdAt)
- [ ] Define `Expense` model (id, category, amount, description, date, vehicleId?, tripId?, organizationId, createdBy, createdAt)
- [ ] Define `Settings` model (id, organizationId, key, value JSON)
- [ ] Define proper relations (foreign keys, cascades)
- [ ] Add composite indexes for common queries (organizationId + status, organizationId + createdAt)
- [ ] Run `npx prisma generate`
- [ ] Run `npx prisma db push` or create initial migration
- [ ] Add seed script with sample data (admin user, org, vehicles, drivers)
